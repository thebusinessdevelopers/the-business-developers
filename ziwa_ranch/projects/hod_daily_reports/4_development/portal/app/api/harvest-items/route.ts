...
interface HarvestRule {
  slugs: string[]
  fields: { fieldName: string; subFieldName: string; category: string }[]
}
...
const HARVEST_RULES: HarvestRule[] = [
  ...
  },
  {
    slugs: ['store'],
    fields: [
      { fieldName: 'goods_added', subFieldName: 'item', category: 'store_goods' },
  ...
  try {
    ...
    for (const fieldRule of rule.fields) {
      ...
      for (const row of rows) {
        const rawName = (row as Record<string, unknown>)[fieldRule.subFieldName]
        if (!rawName || typeof rawName !== 'string') continue

        const itemName = rawName.trim().toLowerCase()
        if (!itemName) continue

        // Upsert: on new item insert; on existing item just update last_seen
